...
def change_content(file_path):
    ...
    with open(file_path, 'w') as file:
        in_function = False
        for line in lines:
            if re.match(r'^\s*function\s', line):    # If the line starts with 'def', it's the start of a function
                in_function = True
...
def copy_and_change(src_directory, dest_directory):
    ...
    shutil.copytree(src_directory, dest_directory, ignore=shutil.ignore_patterns('.DS_Store'))
